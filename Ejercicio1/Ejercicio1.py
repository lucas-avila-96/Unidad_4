...
class Aplicacion(tk.Tk):
    ...
    def __init__(self):
        ...
        tk.Button(
            self, text="Calcular", bg='#5cba5c',fg='#ffffff', height=1,
            width=20, command=self.calcular
            ).grid(column=1, row=3, columnspan=2, sticky=W)

        tk.Button(
            self, text="Limpiar", bg='#5cba5c',fg='#ffffff', height=1,
            width=20, command=self.limpiar
            ).grid(column=3, row=3, columnspan=2, sticky=E) 

        tk.Label(
            self, text=f"Tu Indice de Masa Corporal (IMC) es: ",
            fg='#6d8c68', bg='#e1f0d9'
            ).grid(column=1, row=5, columnspan= 3, sticky=W)

        tk.Label(
            self, textvariable=self.__mci, fg='#6d8c68', 
            bg='#e1f0d9'
        ...
        tk.Label(
            self, textvariable=self.__tipo_mci,
            fg='#6d8c68', bg='#e1f0d9',font=('calibre',10,'bold')
